...
public final class Arez_ComponentTypeNameAloneOnSingletonModel extends ComponentTypeNameAloneOnSingletonModel implements Disposable, Identifiable<Long>, ComponentObservable {
  ...
  public boolean observe() {
    ...
    if ( Arez.shouldCheckApiInvariants() && Arez.areSpiesEnabled() )  {
      Guards.apiInvariant( () -> $$arezi$$_context().isTransactionActive() && $$arezi$$_context().getSpy().getTransaction().isTracking(), () -> "observe method invoked outside a tracking transaction on component of type 'ComponentTypeNameAloneOnSingletonModel'" );
    }
    if ( !isDisposed )  {
