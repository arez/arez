...
final class Arez_ProtectedAccessComponent extends ProtectedAccessComponent implements Disposable, Identifiable<Long>, ComponentObservable {
  ...
  public boolean observe() {
    ...
    if ( Arez.shouldCheckApiInvariants() && Arez.areSpiesEnabled() )  {
      Guards.apiInvariant( () -> $$arezi$$_context().isTransactionActive() && $$arezi$$_context().getSpy().getTransaction().isTracking(), () -> "observe method invoked outside a tracking transaction on component of type 'ProtectedAccessComponent'" );
    }
    if ( !isDisposed )  {
