...
final class Arez_RepositoryWithExplicitNonStandardId extends RepositoryWithExplicitNonStandardId implements Disposable, Identifiable<Integer>, ComponentObservable {
  ...
  public boolean observe() {
    ...
    if ( Arez.shouldCheckApiInvariants() && Arez.areSpiesEnabled() )  {
      Guards.apiInvariant( () -> $$arezi$$_context().isTransactionActive() && $$arezi$$_context().getSpy().getTransaction().isTracking(), () -> "observe method invoked outside a tracking transaction on component of type 'RepositoryWithExplicitNonStandardId'" );
    }
    if ( !isDisposed )  {
