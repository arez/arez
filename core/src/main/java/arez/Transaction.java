...
final class Transaction
{
  ...
  /**
   * Depth counter used to suppress observation tracking during recursive lazy validation.
   */
  private int _observationSuppressionDepth;
  /**
  ...
  void observe( @Nonnull final ObservableValue<?> observableValue )
  {
    ...
    if ( null != _tracker && 0 == _observationSuppressionDepth )
    {
      /*
  ...
  void suppressObservations()
  {
    _observationSuppressionDepth++;
  }

  void restoreObservations()
  {
    assert _observationSuppressionDepth > 0;
    _observationSuppressionDepth--;
  }
