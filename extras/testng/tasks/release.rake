...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/arez/arez-testng/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/arez/arez-testng/compare/v#{ENV['PREVIOUS_PRODUCT_VERSION']}...v#{ENV['PRODUCT_VERSION']})
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      unless candidates.empty?
        ...
        unless milestone[:state] == 'closed'
          client.update_milestone('arez/arez-testng', milestone[:number], :state => 'closed')
        end
      end
    end
  end
