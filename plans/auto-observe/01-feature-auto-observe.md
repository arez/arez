...
9. `validateTypeAtRuntime = true` must only be valid for declared types annotated with `@ArezComponentLike`.
...
- Invalid `validateTypeAtRuntime = true` on non-`@ArezComponentLike` declared types must produce a dedicated processor error.
...
- [ ] Runtime-validation path works only for `@ArezComponentLike`.
