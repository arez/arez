...
- Out of scope: new public marker annotations, renaming `@ActAsComponent`, making `@ActAsComponent` types processable Arez components, and changing existing `@ArezComponent` code generation semantics.

## Locked Decisions

- A covered annotation is misplaced when its nearest enclosing type is annotated with neither `@ArezComponent` nor `@ActAsComponent`.
- `@ActAsComponent` remains a pure marker/exemption type in this change.
- Covered annotations inside `@ActAsComponent` types remain ignored by normal processing and must not trigger the new misplaced-usage error.
...
- Type-level `@SuppressArezWarnings` is only valid when the annotated type itself is annotated with `@ArezComponent` or `@ActAsComponent`; inherited-member exemptions do not apply to the type annotation.
- The validation excludes support/meta annotations `@ArezComponent`, `@ActAsComponent`, and `@DefaultEqualityComparator`.
- Diagnostics should identify the annotation and state that it is only supported within a type annotated by `@ArezComponent` or `@ActAsComponent`.
...
- Add a round-level validation pass before normal `@ArezComponent` processing that:
  ...
  - permits the annotation only when the type has `@ArezComponent` or `@ActAsComponent`,
