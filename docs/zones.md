...
Zones are the mechanism via which multiple independent {@link: arez.ArezContext} instances
...
When zones are enabled, the {@link: arez.Arez#context()} method will return the
{@link: arez.ArezContext} of the current zone. When actions and observers are invoked, Arez will switch to
...
[@ArezComponent](at_arez_component.md) are created, they will invoke {@link: arez.Arez#context()}
and the component will be bound to the current zone. If a method annotated with [@Action](at_action.md) or
[@Observe](at_observe.md) is invoked, the method will switch to the zone in which they were created if is
