...
Arez only re-runs {@link: arez.annotations.Observe @Observe} annotated methods if it is told that an observable property that
is a dependency of the {@link: arez.annotations.Observe @Observe} method is changed. Assuming you are using classes annotated
with {@link: arez.annotations.ArezComponent @ArezComponent} then this means that the code must:

* mutate the property using the setter method to mark the property as changed. The generated code ultimately calls
  the {@link: arez.ObservableValue#reportChanged()} method to mark the property as changed.
* access the property using the getter method.. The generated code ultimately calls the
  {@link: arez.ObservableValue#reportObserved()} method to mark the property as observed.
...
of change but not so in Arez. For example in Arez the {@link: arez.ObservableValue#reportChanged()}
method accepts no change description, the {@link: arez.spy.ObservableValueChangeEvent} class
has no description of a change and there is no way for an {@link: arez.Observer} to receive changes.
...
However building this in Arez with judicious use of {@link: arez.annotations.Memoize @Memoize} annotated
...
The enhanced component classes generated by the annotation processor all implement the {@link: arez.Disposable}. This
...
support {@link: arez.Disposable} but this just lead to resource leaks.
...
{@link: arez.Arez#areNamesEnabled()} returns false) then the base `Object.toString()` method will be invoked.
...
equivalent to Arez's {@link: arez.ObservableValue}, {@link: arez.ComputableValue} and {@link: arez.Observer}.
