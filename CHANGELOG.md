...
* Let frameworks mark their own annotations as Arez-compatible containers via `@ActAsArezComponent`, including a simple-name fallback for downstream package-access meta-annotations that avoid a direct Arez dependency.
* Add `ComponentObservable.maybeObserve()` support method.
