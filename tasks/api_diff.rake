...
task 'do_update_api_diff' do
  a = Buildr.project('arez:core').package(:jar)
  a.invoke
  Buildr::ApiDiffTool.update_differences_report("org.realityforge.arez:arez-core:jar", ENV['PREVIOUS_PRODUCT_VERSION'], ENV['PRODUCT_VERSION'], a.to_s, "#{WORKSPACE_DIR}/api-test", :support_libs => [Buildr.artifact(:javax_annotation).to_s])
end
...
task 'do_test_api_diff' do
  a = Buildr.project('arez:core').package(:jar)
  a.invoke
  Buildr::ApiDiffTool.test_differences_report("org.realityforge.arez:arez-core:jar", ENV['PREVIOUS_PRODUCT_VERSION'], ENV['PRODUCT_VERSION'], a.to_s, "#{WORKSPACE_DIR}/api-test", :support_libs => [Buildr.artifact(:javax_annotation).to_s])
end
