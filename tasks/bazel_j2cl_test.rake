  ...
  class BazelJ2cl
    class << self
      ...
      def perform_bazel_test(root_project, projects, test_module, test_module_file, options)
        root_packages = root_project.packages.select{|p| p.classifier.nil? }.sort { |a, b| b.to_spec <=> a.to_spec }.uniq
        packages = projects.collect { |p| p.packages }.flatten.select{|p| p.classifier.nil? }.sort { |a, b| b.to_spec <=> a.to_spec }.uniq
        all_packages = (root_packages + packages).sort { |a, b| b.to_spec <=> a.to_spec }.uniq
        ...
        write_build(bazel_workspace_dir, test_module, root_packages)
        write_dependency_yml(bazel_workspace_dir, cache_dir, all_packages, options)
