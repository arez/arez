...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/arez/arez/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']})
...
HEADER
    ...
    stage('PushChanges', 'Push changes to git repository') do
      ...
      DOWNSTREAM_PROJECTS.each do |downstream|
        ...
        sh "cd target/arez_downstream-test/deploy_test/workdir/#{downstream} && bundle exec buildr perform_release STAGE=MavenCentralPublish PREVIOUS_PRODUCT_VERSION= PRODUCT_VERSION=#{downstream_version}#{Buildr.application.options.trace ? ' --trace' : ''}"
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('arez/arez', tag, :name => tag, :body => changes, :draft => false, :prerelease => true)
