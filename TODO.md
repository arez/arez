...
* Is there a way to eliminate code generation in `getArezId()` when there should be no callers
  (i.e. requireId=false and compile time repositorys etc disabled?)
...
* Generate a report of API differences between different versions of the API. This report should be uploaded to the
  website and made available via the CHANGELOG.md. It should be based on reports generated by [revapi-diff](https://github.com/realityforge/revapi-diff)
  in `api-tests` package.
