...
abstract class CustomStorePersistModel_PersistSidecar {
  ...
  @Nonnull
  static CustomStorePersistModel_PersistSidecar attach(
      @Nonnull final Scope scope, @Nonnull final CustomStorePersistModel peer) {
    ...
    Arez.context()
        .task(
            Arez.areNamesEnabled()
                ? "CustomStorePersistModel_PersistSidecar.attach." + (++c_nextTaskId)
                : null,
  ...
  void restoreState() {
    ...
    if (!_sStore.isDisposed() && Disposable.isNotDisposed(_scope)) {
      final Map<String, Object> state =
          _sStore.get(_scope, Keys.TYPE, $ap$_id, Converters.TYPE_CONVERTER);
      if (null != state) {
  ...
  private static final class Keys {
    ...
    private static final String TYPE = Arez.areNamesEnabled()
        ? "CustomStorePersistModel"
        : CustomStorePersistModel.class.getName();
