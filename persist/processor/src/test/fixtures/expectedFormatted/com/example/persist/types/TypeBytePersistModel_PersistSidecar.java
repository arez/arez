...
abstract class TypeBytePersistModel_PersistSidecar {
  ...
  TypeBytePersistModel_PersistSidecar(
  ...
  @Nonnull
  static TypeBytePersistModel_PersistSidecar attach(
      @Nonnull final Scope scope, @Nonnull final TypeBytePersistModel peer) {
    assert Disposable.isNotDisposed(scope);
    assert Disposable.isNotDisposed(peer);
    final Store appStore = ArezPersist.getStore("app");
    return new Arez_TypeBytePersistModel_PersistSidecar(scope, peer, appStore);
  }
  ...
  @Nonnull
  static void scheduleAttach(@Nonnull final Scope scope, @Nonnull final TypeBytePersistModel peer) {
    assert Disposable.isNotDisposed(scope);
    assert Disposable.isNotDisposed(peer);
    Arez.context()
        .task(
            Arez.areNamesEnabled()
                ? "TypeBytePersistModel_PersistSidecar.attach." + (++c_nextTaskId)
                : null,
  ...
  private static final class Keys {
    ...
    private static final String TYPE =
        Arez.areNamesEnabled() ? "TypeBytePersistModel" : TypeBytePersistModel.class.getName();
  ...
  private static final class Converters {
    @Nonnull
    private static final Converter CONVERTER_byte = ArezPersist.getConverter(byte.class);
    ...
    private static TypeConverter createTypeConverter() {
      ...
      converters.put("value", CONVERTER_byte);
