...
abstract class TypeDoublePersistModel_PersistSidecar {
  ...
  TypeDoublePersistModel_PersistSidecar(
      @Nonnull final Scope scope,
      @Nonnull final TypeDoublePersistModel peer,
  ...
  @Nonnull
  static TypeDoublePersistModel_PersistSidecar attach(
      @Nonnull final Scope scope, @Nonnull final TypeDoublePersistModel peer) {
    assert Disposable.isNotDisposed(scope);
    assert Disposable.isNotDisposed(peer);
    final Store appStore = ArezPersist.getStore("app");
    return new Arez_TypeDoublePersistModel_PersistSidecar(scope, peer, appStore);
  }
    ...
    Arez.context()
        .task(
            Arez.areNamesEnabled()
                ? "TypeDoublePersistModel_PersistSidecar.attach." + (++c_nextTaskId)
                : null,
  ...
  void restoreState() {
    ...
    if (!_appStore.isDisposed() && Disposable.isNotDisposed(_scope)) {
      ...
      if (null != state) {
        final Double $prop$_value = (Double) state.get(Keys.PROPERTY_value);
        if (null != $prop$_value) {
  ...
  void persistState() {
    if (!_appStore.isDisposed() && Disposable.isNotDisposed(_scope)) {
      ...
      final double $prop$_value = _peer.getValue();
      if (0.0 != $prop$_value) {
  ...
  private static final class Keys {
    ...
    private static final String TYPE =
        Arez.areNamesEnabled() ? "TypeDoublePersistModel" : TypeDoublePersistModel.class.getName();
  ...
  private static final class Converters {
    @Nonnull
    private static final Converter CONVERTER_double = ArezPersist.getConverter(double.class);
    ...
    private static TypeConverter createTypeConverter() {
      ...
      converters.put("value", CONVERTER_double);
